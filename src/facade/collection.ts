...
/* SEE: https://github.com/angular/angular/blob/master/modules/angular2/src/core/facade/collection.ts */

import {isJsObject, global, isPresent, isBlank, isArray} from './lang';
...
var createMapFromPairs: {(pairs: any[]): Map<any, any>} = (function() {
    try {
        if (new Map(<any>[[1, 2]]).size === 1) {
            return function createMapFromPairs(pairs: any[]):
...
export class ListWrapper {
    ...
    static get<T>(m: T[], k: number): T { return m[k]; }
    static set<T>(m: T[], k: number, v: T) { m[k] = v; }
